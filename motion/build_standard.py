...
def write_elements_csv(path, elements):
    with open(path, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["element", "cycle", "position", "start_s", "end_s", "dur_s",
                    "signature", "support", "grasp_x", "grasp_y", "release_x", "release_y"])
        for e in elements:
            d = e.to_dict()
            ...
            w.writerow([d["index"] + 1,
                        ...
                        d["start_s"], d["end_s"], d["dur_s"], d["signature"], d["support"],
                        g[0], g[1], r[0], r[1]])
...
def write_standard_csv(path, std, rating, allowance):
    with open(path, "w", newline="", encoding="utf-8") as fh:
        w = csv.writer(fh)
        w.writerow(["순번", "동작구성", "중앙값(초)", "IQR(초)", "최소", "최대",
                    "출현", "정미시간(초)", "표준시간(초)", "보조손", "비고"])
        for s in std["standard"]:
            net = s["median_s"] * rating
            w.writerow([s["position"], s["signature"], s["median_s"], s["iqr_s"],
